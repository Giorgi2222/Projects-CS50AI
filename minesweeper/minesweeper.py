...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        self.mark_safe(cell)
        cells = set()
        if (cell[0]+1, cell[1]) in self.all_cells:
            cells.add((cell[0]+1, cell[1]))
        if (cell[0]-1, cell[1]) in self.all_cells:
            cells.add((cell[0]-1, cell[1]))
        if (cell[0]+1, cell[1]-1) in self.all_cells:
            cells.add((cell[0]+1, cell[1]-1))
        if (cell[0], cell[1]-1) in self.all_cells:
        ...
        for sentence1 in self.knowledge:
            for sentence2 in self.knowledge:
                if sentence1.cells.issubset(sentence2.cells) and sentence1.count != sentence2.count:
                    temp_count = sentence2.count - sentence1.count
                    temp_cells = set([cell for cell in sentence2.cells if cell not in sentence1.cells])
                    temp_knowledge.append(Sentence(cells = temp_cells, count = temp_count))
